...
import { Newspaper, Plus, RefreshCw, Search, X, ExternalLink, Rss, ChevronDown } from 'lucide-react';
import { api } from './api';
  ...
  return (
    <div className="empty-state">
      ...
      <p className="empty-state-text">
        Добавьте RSS-источники, и мы автоматически создадим для вас краткие дайджесты с помощью ИИ.
      </p>
...
function SkeletonCard({ index }) {
  return (
    <div className="skeleton-card" style={{ animationDelay: `${index * 80}ms` }}>
      <div className="skeleton skeleton-line short" style={{ animationDelay: `${index * 80}ms` }} />
      <div className="skeleton skeleton-title" style={{ animationDelay: `${index * 80 + 50}ms` }} />
      <div className="skeleton skeleton-line medium" style={{ animationDelay: `${index * 80 + 100}ms` }} />
      <div className="skeleton skeleton-line medium" style={{ animationDelay: `${index * 80 + 150}ms` }} />
      <div className="skeleton skeleton-line short" style={{ animationDelay: `${index * 80 + 200}ms` }} />
    </div>
  );
}
        ...
        <div className="feeds-dropdown-menu">
          ...
          {feeds.map((feed) => (
            <div
              key={feed.id}
              className="feeds-dropdown-item"
              onClick={() => { onSelect(feed.id); setIsOpen(false); }}
            >
              ...
              <div className="stats">
                <span className="stat">
                  ...
                  {articles.length} {articles.length === 1 ? 'статья' : 
                    articles.length < 5 ? 'статьи' : 'статей'}
                </span>
              </div>
